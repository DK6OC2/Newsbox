...
function msg_list()
{
	...
	$statement = $db->prepare('SELECT * FROM messages ORDER BY validfrom DESC');
	$statement->execute();
	$rows = [];
...
function topic_list()
{
	...
	$statement = $db->prepare('SELECT * FROM topics ORDER BY id ASC');
	$statement->execute();
	$rows = [];
	while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
		$rows[] = $row;
...
function topic_insert($arr_content)
{
	global $db;
	if (empty($db)) {
		return false;
